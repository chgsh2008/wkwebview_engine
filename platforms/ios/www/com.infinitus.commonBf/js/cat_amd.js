        ...
        catAnalyticsEnd: function(options, returnOptions, msg) {
            ...
            for (var i = 0; i < ajaxAnalyticObject.length; i++) {
                if (ajaxAnalyticObject[i].id == options) {
                    ...
                    category = $("[data-click-title]").attr("data-click-title") || $("header .text-center").text() || ajaxUrl;
                    //catAnalytics(category, "耗时:" + _time + "ms", ajaxUrl, msg);
                    //url为file开头的不上报
                    if (ajaxUrl.match(/^(?!file).*/)) {
                        setTimeout(function() {
                            bsl.infinitus.cat.monitor({
