 ...
 *
 */
define(['zepto', 'underscore', 'com.infinitus.butterfly/components/loader', 'com.infinitus.butterfly/components/cache', 'com.infinitus.butterfly/components/store',
        'iScroll', 'backbone', 'i18n!com.infinitus.butterfly/components/nls/cube-calendar-local'
    ],
    function($, _, Loader, Cache, Store, iScroll, Backbone, Locale) {
        ...
        var List = Backbone.View.extend({
                ...
                if (this.config.iScroll != 'false') {
                    ...
                    var list_iScroll = new iScroll(this.el, {
                        onBeforeScrollStart: function(e) {
                            ...
                            if (target.tagName != 'DIV' && target.tagName != 'IMG' && target.tagName != 'TEXTAREA' && target.tagName != 'INPUT' && target.tagName != 'SELECT') e.preventDefault();
                        },
            ...
            loadListByJSONArray: function(jsonArray, isLoadFromStore) {
                ...
                if (jsonArray === undefined || jsonArray === null || jsonArray.length === 0) {
                    if (me.$('.cube-list-item-more-record').length === 0) {
                        ...
                        // li.html('无相关记录');
                        li.html(Locale.NoRelatedRecords);
                ...
                for (var i = 0; i < jsonArray.length; i++) {
                    ...
                    if (_itemTemplateName) {
                        var templateHtml = _.template(templateStr, item) ;
                        // console.info("templateHtml....") ;
                        // console.info(templateHtml) ;
                        li.append(_.template(templateStr, item))
                    };
                    //TODO: 需要重构
                    li.appendTo(me.el.querySelector('.contentScroller'));
            ...
            isExistStoreData: function(listName) {
                if (window.localStorage["cube-list-" + listName] === undefined || window.localStorage["cube-list-" + listName] === null || window.localStorage["cube-list-" + listName] === "null") {
                    return false;
                } else {
                    return true;
                }
            ...
            loadListByStoreData: function(url, params) {
                ...
                /*如果参数变了 重新向服务器请求数据 by kebin luohanwen*/
                var paramsOrign = params;
                var paramsTemp = tempparams;
                paramsTemp.page = 1;
                paramsOrign.page = 1;
                var paramsValue = JSON.stringify(JSON.stringify(paramsOrign));
                var tempparamsValue = JSON.stringify(JSON.stringify(paramsTemp));

                if (paramsValue !== tempparamsValue) {
            ...
            loadNextPage: function(callback) {
                ...
                $.ajax({
                    ...
                    success: function(data, textStatus, jqXHR) {
                        ...
                        /*增加国际化及callback支持 by luohanwen*/
                        //缓存国际化优化（Zping on 2140522)
                        var i18nData = me['i18n'];
                        if (me.config['successCallback'] !== null && me.config['successCallback'] !== undefined) {
                            me.config['successCallback'](data, textStatus, jqXHR);
                        }
                    ...
                    error: function(e, xhr, type) {
                        ...
                        loader.hide();
                        //回调errror by luohanwen
                        if (me.config['errorCallback'] !== null && me.config['errorCallback'] !== undefined) {
                            me.config['errorCallback'](e, xhr, type);
                        }
                    }
                });
            }
